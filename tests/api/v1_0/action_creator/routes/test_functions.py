...
def test_presets_endpoint_returns_all_presets(client: TestClient, auth_token: str) -> None:
    response = client.get("/api/v1.0/action-creator/presets", headers={"Authorization": f"Bearer {auth_token}"})
    assert response.status_code == status.HTTP_200_OK
    assert len(response.json()["presets"]) > 1


def test_presets_endpoint_returns_presets_for_collection(client: TestClient, auth_token: str) -> None:
    collection = "sentinel-2-l2a"
    response = client.get(
        f"/api/v1.0/action-creator/presets?collection={collection}",
        headers={"Authorization": f"Bearer {auth_token}"},
    )
    assert response.status_code == status.HTTP_200_OK
    for f in response.json()["presets"]:
        assert collection in f["inputs"]["stac_collection"]["options"]


def test_presets_endpoint_returns_empty_function_list(client: TestClient, auth_token: str) -> None:
    ...
    assert response.status_code == status.HTTP_200_OK
    assert not response.json()["presets"]
...
def test_functions_endpoint_returns_functions_for_collection(client: TestClient, auth_token: str) -> None:
    ...
    for f in response.json()["functions"]:
        if "stac_collection" in f["inputs"] and "options" in f["inputs"]["stac_collection"]:
            assert collection in f["inputs"]["stac_collection"]["options"]
