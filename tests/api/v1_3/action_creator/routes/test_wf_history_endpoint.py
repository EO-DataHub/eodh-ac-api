...
def test_get_job_submissions_endpoint_returns_valid_response_no_params(
    ...
    # Act
    response = client.get(
        "/api/v1.3/action-creator/workflow-submissions", headers={"Authorization": f"Bearer {auth_token_module_scoped}"}
    )
...
def test_get_job_submissions_with_custom_per_page_item_count_and_page_idx(
    ...
    # Act
    response = client.get("/api/v1.3/action-creator/workflow-submissions", headers=headers, params=params)
