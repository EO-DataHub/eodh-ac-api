...
def test_get_job_submissions_endpoint_returns_valid_response_no_params(
    ...
    assert pagination_results.results_on_current_page == DEFAULT_RESULTS_PER_PAGE
    assert pagination_results.ordered_by == "submitted_at"
    assert pagination_results.order_direction == OrderDirection.asc
    assert pagination_results.total_pages == math.ceil(TOTAL_ITEMS / DEFAULT_RESULTS_PER_PAGE)
    assert len(pagination_results.results) == DEFAULT_RESULTS_PER_PAGE
...
def test_get_job_submissions_with_custom_per_page_item_count_and_page_idx(
    ...
    # Act
    response = client.get("/api/v1.0/action-creator/submissions", headers=headers, params=params)
